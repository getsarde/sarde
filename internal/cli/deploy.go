...
var deployCmd = &cobra.Command{
	...
	Long:  "Deploy the site output directory to a hosting provider. Run 'coderoo build' first.",
	RunE:  runDeploy,
}
...
func runDeploy(cmd *cobra.Command, args []string) error {
	...
	if _, err := os.Stat(outputDir); os.IsNotExist(err) {
		return fmt.Errorf("output directory %q does not exist; run 'coderoo build' first", outputDir)
	}
