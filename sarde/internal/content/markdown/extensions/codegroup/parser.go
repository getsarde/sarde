...
var openingRegex = regexp.MustCompile(`^:{3,}\s*code-group\s*$`)
var closingRegex = regexp.MustCompile(`^:{3,}(?:/([\w-]+))?\s*$`)
var nestedOpenRegex = regexp.MustCompile(`^:{3,}\s*\w+`)

type codeGroupParser struct{}

// NewParser returns a new code group block parser.
func NewParser() parser.BlockParser {
	return &codeGroupParser{}
}
...
func (p *codeGroupParser) Open(parent ast.Node, reader text.Reader, pc parser.Context) (ast.Node, parser.State) {
	...
	return &CodeGroupBlock{}, parser.HasChildren
}
...
func (p *codeGroupParser) CanInterruptParagraph() bool {
	return false
}

func (p *codeGroupParser) CanAcceptIndentedLine() bool {
	return false
}
