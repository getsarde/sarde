...
var badgeVariants = []string{"tip", "note", "danger", "warning"}
...
func standardBody(rng *rand.Rand, sections int, domain string) string {
	var b strings.Builder
	topicPool := docsTopics
	if sections <= 0 {
		sections = 4 + rng.Intn(3)
	}
	for i := 0; i < sections; i++ {
		topic := pick(rng, topicPool)
...
func docsFrontmatter(title, desc string, weight int, tags []string, badge string) string {
	...
	if badge != "" {
		...
		b.WriteString(fmt.Sprintf("badge:\n  text: %q\n  variant: %q\n", badge, variant))
	}
	b.WriteString("---\n\n")
	return b.String()
}
...
func sectionFrontmatter(title, desc string, weight int, icon string) string {
	var b strings.Builder
	b.WriteString("---\n")
	b.WriteString(fmt.Sprintf("title: %q\n", title))
	b.WriteString(fmt.Sprintf("description: %q\n", desc))
...
func sardeYAML() string {
	return `site:
  title: "Sarde Benchmark Site"
  description: "A 1000-page benchmark site for testing Sarde build performance."
...
deployment:
  label: "Deployment"
  description: "Deployment strategies and CI/CD"
  color: "#0891b2"
testing:
  label: "Testing"
...
plugins:
  label: "Plugins"
  description: "Plugin development and usage"
  color: "#d946ef"
api:
  label: "API"
  description: "API design and reference"
  color: "#f43f5e"
middleware:
  label: "Middleware"
...
func enI18n() string {
  ...
  language: "Language"
fallback:
  notice: "This page is not yet available in {{ .Lang }}."
search:
...
func generateBlog(rng *rand.Rand, root string) int {
	contentRoot := filepath.Join(root, "content", "blog")
	mustWrite(root, filepath.Join("content", "blog", "_index.md"),
		sectionFrontmatter("Blog", "Latest posts and articles from the team.", 0, ""))
	count := 1
...
func generateArticles(rng *rand.Rand, root string) int {
	mustWrite(root, filepath.Join("content", "articles", "_index.md"),
		sectionFrontmatter("Articles", "In-depth articles on software engineering.", 0, ""))
	count := 1
...
func generateNews(rng *rand.Rand, root string) int {
	mustWrite(root, filepath.Join("content", "news", "_index.md"),
		sectionFrontmatter("News", "Short updates and announcements.", 0, ""))
	count := 1
...
func generateDocs(rng *rand.Rand, root string) int {
	mustWrite(root, filepath.Join("content", "docs", "_index.md"),
		sectionFrontmatter("Documentation", "Complete reference documentation.", 0, "book"))
	count := 1

	for si, sec := range docsSectionDefs {
		secDir := filepath.Join("content", "docs", sec.Dir)
		mustWrite(root, filepath.Join(secDir, "_index.md"),
			sectionFrontmatter(sec.Title, fmt.Sprintf("Documentation for %s.", sec.Domain), (si+1)*10, sec.Icon))
		count++
...
func generateTutorials(rng *rand.Rand, root string) int {
	mustWrite(root, filepath.Join("content", "tutorials", "_index.md"),
		sectionFrontmatter("Tutorials", "Hands-on tutorials to learn by doing.", 0, "graduation-cap"))
	count := 1

	for si, sec := range tutorialSectionDefs {
		secDir := filepath.Join("content", "tutorials", sec.Dir)
		mustWrite(root, filepath.Join(secDir, "_index.md"),
			sectionFrontmatter(sec.Title, fmt.Sprintf("Tutorials about %s.", sec.Title), (si+1)*10, ""))
		count++
...
func generateCourses(rng *rand.Rand, root string) int {
	mustWrite(root, filepath.Join("content", "courses", "_index.md"),
		sectionFrontmatter("Courses", "Structured courses for all skill levels.", 0, "school"))
	count := 1

	for si, sec := range courseSectionDefs {
		secDir := filepath.Join("content", "courses", sec.Dir)
		mustWrite(root, filepath.Join(secDir, "_index.md"),
			sectionFrontmatter(sec.Title, fmt.Sprintf("A complete course on %s.", sec.Title), (si+1)*10, ""))
		count++
...
func generateStandalones(root string) int {
	pages := []struct {
		...
		{"_index.md", "Sarde Benchmark", "A 1000-page benchmark site for Sarde.", "Welcome to the Sarde benchmark site.\n"},
		{"about.md", "About", "About this benchmark site.", "## About\n\nThis site exists to benchmark the Sarde static site generator build pipeline.\n"},
	...
	for _, p := range pages {
		fm := sectionFrontmatter(p.title, p.desc, 0, "")
...
func generateFR(rng *rand.Rand, root string) int {
	...
	mustWrite(root, filepath.Join("content", "fr", "_index.md"),
		sectionFrontmatter("Accueil", "Site de benchmark Sarde en français.", 0, ""))
	count++

	mustWrite(root, filepath.Join("content", "fr", "blog", "_index.md"),
		sectionFrontmatter("Blog", "Articles et publications de l'équipe.", 0, ""))
	count++
	...
	mustWrite(root, filepath.Join("content", "fr", "docs", "_index.md"),
		sectionFrontmatter("Documentation", "Documentation complète en français.", 0, "book"))
	count++
	...
	for si, sec := range frDocsSections {
		...
		mustWrite(root, filepath.Join(secDir, "_index.md"),
			sectionFrontmatter(sec.title, fmt.Sprintf("Section %s en français.", sec.title), (si+1)*10, ""))
		count++
...
func main() {
	...
	root := flag.String("root", "", "output root directory (default: auto-detect from script location)")
	...
	if *root == "" {
		...
		// (benchmarks/generators/sarde); under `go run` the executable lives
		// in the build cache, so always pass -root explicitly there.
		exe, err := os.Executable()
		if err == nil {
			candidate := filepath.Join(filepath.Dir(exe), "..", "..", "fixtures", "sarde")
	...
	fmt.Printf("Generating benchmark site in %s ...\n", *root)

	// Write config files.
	mustWrite(*root, "sarde.yaml", sardeYAML())
	mustWrite(*root, filepath.Join("data", "tags.yml"), tagsYML())
	mustWrite(*root, filepath.Join("i18n", "en.yaml"), enI18n())
	mustWrite(*root, filepath.Join("i18n", "fr.yaml"), frI18n())
	...
	fmt.Printf("\n  Total:         %d pages\n", total)
	fmt.Println("\nConfig files: sarde.yaml, data/tags.yml, i18n/en.yaml, i18n/fr.yaml")
}
