...
import (
	...
	"github.com/coderoo-dev/coderoo/internal/download"
...
func runThemeAdd(cmd *cobra.Command, args []string) error {
	...
	if _, err := os.Stat(destDir); err == nil {
		return fmt.Errorf("theme %q already exists; remove it first with 'coderoo theme remove %s'", name, name)
	}
	...
	if thm == nil {
		...
		return fmt.Errorf("installed directory does not contain a valid theme.yaml; not a valid coderoo theme")
	}
...
func installFromGitHub(src, destDir string) error {
	...
	scratchDir, err := os.MkdirTemp("", "coderoo-theme-extract-*")
	if err != nil {
